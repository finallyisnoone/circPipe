...
volcano_plot <- function(res_df,contrast_factor,ylab_variable="FDR",pval = 0.05,fc=1.5){
  ...
  mtext(c(paste("-", fc), paste("+", fc)), side = 1, at = c(-lfc, lfc), cex = 0.8, line = 0.5)
...
get_pheno <- function(x,label1,label2,group1,group2){
  ...
  # temp_T <-data.frame(sample = x[grep(label1,x)],Type=rep(group1,length(x[grep(label2,x)])))
  # temp_N <-data.frame(sample = x[grep(label2,x)],Type=rep(group2,length(x[grep(label2,x)])))
  # pheno = as.data.frame(rbind(temp_T,temp_N))
  pheno_data = as.data.frame(Type = c(rep(group1,length(grep(label2,x))),
